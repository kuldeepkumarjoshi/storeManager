...
public final class QueryString {
	...
	public static final String ORDER_DETAIL_WITH_PRODUCT_DATE_WISE = "select opi.id,opi.total,opi.status,pr.name from ("+
			"select oi.id,oi.total,oi.status, op.productId from orderItem oi Inner join orderProduct op on oi.id = op.orderId"+ 
	...
	public static final String ORDER_DETAIL_WITH_PRODUCT_STATUS_WISE = "select opi.id,opi.total,opi.status,pr.name from ("+
			"select oi.id,oi.total,oi.status, op.productId from orderItem oi Inner join orderProduct op on oi.id = op.orderId"+ 
	...
	public static final String ORDER_DETAIL_WITH_PRODUCT_STORE_WISE = "select opi.id,opi.total,opi.status,pr.name from ("+
			"select oi.id,oi.total,oi.status, op.productId from orderItem oi Inner join orderProduct op on oi.id = op.orderId"+ 
