...
(function(define) {
    ...
    module.directive('uiBreadcrumbs', function($interpolate, $state) {
            return {
                ...
                template : '<ul class="breadcrumb"><i class="ace-icon fa fa-home home-icon"></i><li ng-if="breadcrumbs[0].displayName != \'Dashboard\' && isDashboardShow != false">aman{{isDashboardShow}}<a href="/client/index-ace.html#/{{dashboardURL}}" >Dashboard</a></li><li ng-repeat="crumb in breadcrumbs" ng-class="{ active: $last }"><a ui-sref="{{ crumb.route }}" ng-if="!$last">{{crumb.displayName}}</a><span ng-show="$last">{{crumb.displayName }}</span></li></ul>',
                scope: {
                    displaynameProperty: '@',
